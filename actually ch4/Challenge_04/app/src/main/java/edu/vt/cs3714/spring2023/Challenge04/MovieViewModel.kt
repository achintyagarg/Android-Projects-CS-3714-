...
class MovieViewModel (application: Application) : AndroidViewModel(application){
    ...
    private val api_key = "92d3cae285296ce34140538f0b7f5671"
    ...
    fun startRefreshMovies() {
        deleteAll()
        refreshMoviePage(1)
    }

    private fun refreshMoviePage(page: Int) {
        disposable =
            RetrofitService.create(api_base_url).getNowPlaying(api_key, page)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ result ->
                    showResult(result)
                    if (page < 3) {
                        refreshMoviePage(page + 1)
                    }
                }, { error ->
                    showError(error)
                })
